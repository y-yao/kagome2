...
protected:
    ...
    const long int m_n;            // dimension of A
...
#pragma omp parallel for schedule(static)
	for (size_t i = 0; i < v.size(); i++) v(i) = v0(i) / v0norm;
...
#pragma omp parallel for schedule(static)
	for (size_t i = 0; i < m_fac_f.size(); i++) m_fac_f(i) = w(i) - v(i) * m_fac_H(0, 0);
...
//            Vs.col(i).noalias() = m_fac_V.leftCols(nnz) * q;
	    m_op.matvec_product(m_fac_V.leftCols(nnz), q, Vs.col(i));
        }
        //Vs.col(m_k).noalias() = m_fac_V * Q.col(m_k);
        //m_fac_V.leftCols(m_k + 1).noalias() = Vs;
	m_op.matvec_product(m_fac_V, Q.col(m_k), Vs.col(m_k));
#pragma omp parallel for schedule(static)
	for (int k = 0; k < m_k + 1; k++) {
            m_fac_V.col(k).noalias() = Vs.col(k);
	}

        //Vector fk = m_fac_f * Q(m_m - 1, m_k - 1) + m_fac_V.col(m_k) * m_fac_H(m_k, m_k - 1);
	Vector fk(m_n);
#pragma omp parallel for schedule(static)
	for (long int k = 0; k < m_n; k++) fk(k) = m_fac_f(k) * Q(m_m - 1, m_k - 1) + m_fac_V(k, m_k) * m_fac_H(m_k, m_k - 1); 
        m_fac_f.swap(fk);
        m_beta = m_op.norm(m_fac_f);
    }
};
