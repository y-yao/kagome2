...
#pragma omp parallel for schedule(static)
	    for (size_t x = 0; x < m_fac_f.rows(); x++) {
                v(x) = m_fac_f(x) / m_beta;
	    }
...
#pragma omp parallel for schedule(static)
                for (size_t k = 0; k < m_fac_f.size(); k++) m_fac_f(k) = w(k) - m_fac_H(i, i) * v(k);
            else
                //m_fac_f.noalias() = w - m_fac_H(i, i - 1) * m_fac_V.col(i - 1) - m_fac_H(i, i) * v;
#pragma omp parallel for schedule(static)
                for (size_t k = 0; k < m_fac_f.size(); k++) m_fac_f(k) = w(k) - m_fac_H(i, i - 1) * m_fac_V(k, i - 1) - m_fac_H(i, i) * v(k);
...
#pragma omp parallel for
	    ...
	    std::cout << "\titer" <<i<<" in Lanczos.factorize_from " << std::chrono::duration<double>(end-start).count() << "s." << std::endl;
