...
protected:
    ...
    const long int    m_n;          // dimension of matrix A
...
private:
    ...
    void restart(int k)
    {
        ...
        std::cout << "m_fac.factorize_from in restart takes " << std::chrono::duration<double>(end-start).count() << "s." << std::endl;
...
public:
	...
	retrieve_ritzpair();
	    ...
	    std::cout << "Start of restart" << std::endl;
            restart(nev_adj);
            auto end3 = std::chrono::high_resolution_clock::now();
	    std::cout << "restart takes " << std::chrono::duration<double>(end3-start3).count() << "s." << std::endl;
...
#pragma omp parallel for schedule(static)
	        ...
	        std::cout << "Writing eigen_progress takes " << std::chrono::duration<double>(end4-start4).count() << "s." << std::endl;
	    }
        }
        // Sorting results
        sort_ritzpair(sort_rule);
